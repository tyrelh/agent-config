...
8. Always set created tickets to the built-in default team unless the user explicitly requests a different team.
9. "Team" in the UI maps to `group_id` in the API; list groups to find the team ID.
10. For state updates, update the story with the target `workflow_state_id`.
...
- When taking on a ticket, always ensure it's assigned to the user, gets put in the current iteration, and it gets moved to in-progress appropriately.
  - To put a ticket in the current iteration: run `shortcut_get_current_iteration.sh` to find the active iteration ID, then run `shortcut_set_iteration.sh <story_id> <iteration_id>`.
- When creating a ticket, always set the team to the built-in default unless the user explicitly requests a different team.
- When creating a ticket, always set the **Technical Area** custom field. Pick it from where the work lands: `Lightrail API` for `lightrail` Go work, `Lightrail FE` for `giftbitfe`, `v1 Webapp/Batch` for `giftbitapp`. Pass it as arg 7 of the create script, or set it afterwards with `shortcut_set_custom_field.sh`.
...
- `scripts/shortcut_list_custom_fields.sh [name_regex]`
- `scripts/shortcut_set_custom_field.sh <story_id> <field_name_or_id> <value_name_or_id>`
...
- `SHORTCUT_GROUP_ID` (optional, used by update/create scripts; overrides the built-in default during create)
- `SHORTCUT_OWNER_IDS` (optional, used by update/create scripts)
- `SHORTCUT_TECHNICAL_AREA` (optional, used by create script; a Technical Area value name or ID)
- `SHORTCUT_EPIC_ID` (optional, used by create script)
...
- Provide either `project_id` or `workflow_state_id`. If projects are not used, pick a workflow state from the workflows list (typically **Ready for Development**).
- Use the create script and include `owner_ids` if needed.
- Always set the ticket team to the built-in default unless the user explicitly requests a different team. The create script uses this default when `group_id` and `SHORTCUT_GROUP_ID` are unset.
- Ticket **Title:**: one concise line — outcome or problem, not implementation steps.
- Ticket **Description:**: use this markdown structure:

```markdown
## Description

<Full description of the problem or feature — what needs to change, why it matters, and any relevant context.>

## Acceptance Criteria

- One top-level bullet per verifiable outcome (Definition of Done)
- Nest sub-bullets for related details: tests, metrics, migrations, docs
- Be specific — name fields, endpoints, metrics, behaviors
- Example:
  - Add the `active` field to the `Product` spec and persist it
    - Add unit and integration tests for this change
    - Emit the Datadog metric `ProductIsActive` when a product is activated
```
...
## Task: Set a Custom Field

- Use for **Technical Area** (required on every created ticket), Priority, Skill Set, or Severity.
- `shortcut_set_custom_field.sh` takes names or raw UUIDs and resolves them against `/custom-fields`, so `shortcut_set_custom_field.sh 24806 "Technical Area" "Lightrail API"` works directly. Matching is case-insensitive, and an unknown name errors with the list of valid options.
- Run `shortcut_list_custom_fields.sh` first when you are unsure which values exist.
- On create, pass the Technical Area as arg 7 instead of making a second call.
- Note: `Product Area` is disabled in this workspace; setting it fails.

### API Example (custom field)

```bash
curl -sS -X PUT "$SHORTCUT_API_BASE_URL/stories/<story_id>" \
  -H "Shortcut-Token: $SHORTCUT_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"custom_fields":[{"field_id":"<field_uuid>","value_id":"<value_uuid>"}]}'
```
...
- Use when you need to find an iteration by name or list all iterations.
- Supports optional name filtering via regex.

### API Example (iterations)

```bash
curl -sS "$SHORTCUT_API_BASE_URL/iterations" \
  -H "Shortcut-Token: $SHORTCUT_API_TOKEN"
```

Use `scripts/shortcut_list_iterations.sh` for a debuggable version of this call.

## Task: Get Current Iteration

- Use when you need to find the active/started iteration (e.g., to assign a story to the current sprint).
- Lists all iterations and filters for `status == "started"`.
- Exits with code 1 if no current iteration is found.

Use `scripts/shortcut_get_current_iteration.sh` to find the current iteration.

## Task: Set Story Iteration

- Use when you need to assign a story to an iteration (sprint).
- First find the target iteration ID using `shortcut_get_current_iteration.sh` or `shortcut_list_iterations.sh`.
- Then update the story with the iteration ID.
- Supports `SHORTCUT_DRY_RUN=1` to preview the payload without making changes.

### API Example (set iteration)

...
### Available skills
- codex-delegate: Delegate a subtask to the local codex CLI instead of doing it in this session. Use when the user says "get codex to do X", "ask codex to…", "have codex…", "delegate this to codex", or when a chunk of work is self-contained enough to run on codex's separate quota. Returns codex's final message and a session id for follow-up work. (file: /Users/tyrel/.codex/skills/codex-delegate/SKILL.md)
